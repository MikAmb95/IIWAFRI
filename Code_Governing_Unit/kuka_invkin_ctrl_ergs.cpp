...
class KUKA_INVKIN {
	public:
		...
		bool init_robot_model();
                double staturation(double zeta,double L, double M);
                VectorXd lyp(VectorXd vel,MatrixXd B,VectorXd e,VectorXd pos);
                double integral(double a,double b,double n,int index,VectorXd pos);
                double f_integ_1(double r1,int index,VectorXd pos); 
		void ctrl_loop();
                double ERG_function();
...
double KUKA_INVKIN::staturation(double zeta,double L, double M){


double lim_p = L + (M-L)*tanh((zeta-L)/(M-L));
double lim_m = L + (M-L)*tanh((zeta-L)/(M-L));
...
double KUKA_INVKIN::f_integ_1(double r1,int index,VectorXd pos){
...
q_in4 = new KDL::JntArray( _k_chain.getNrOfJoints() );

q_in3->data = _v_q->data + q0->data; //q_desided

_dyn_param->JntToGravity(*q_in3, grav_2);

double x  = grav_2.data(index)-Kp(index,index)*r1;

double s = 0;

s = KUKA_INVKIN::staturation(x,Ll[index],Mm[index]);

KDL::JntArray grav_3(7);

grav_3.data = grav_2.data;
grav_3.data(index) = grav_3.data(index)+r1;

if(index == 0){
q_in4->data[0] = r1 + q_in3->data[0];
q_in4->data[1] = q_in3->data[1];
q_in4->data[2] = q_in3->data[2];
q_in4->data[3] = q_in3->data[3];
q_in4->data[4] = q_in3->data[4];
q_in4->data[5] = q_in3->data[5];
q_in4->data[6] = q_in3->data[6];

}

else if(index == 1){
q_in4->data[0] = pos(0);
q_in4->data[1] = r1 + q_in3->data[1];
q_in4->data[2] = q_in3->data[2];
q_in4->data[3] = q_in3->data[3];
q_in4->data[4] = q_in3->data[4];
q_in4->data[5] = q_in3->data[5];
...
_dyn_param->JntToGravity(*q_in4, grav_3);

double f = 0;

f = grav_3.data(index) - s;
...
double KUKA_INVKIN::integral(double a,double b,double n,int index,VectorXd pos) {
	...
	for(int i=0;i<n;i++){
                ...
                r = a + (i + 0.5)*step;
	        x  = KUKA_INVKIN::f_integ_1(r,index,pos);
		area = area + x*step; //function f = x;
...
VectorXd KUKA_INVKIN::lyp(VectorXd vel,MatrixXd B,VectorXd e,VectorXd pos) {
	...
	VectorXd V1;
        ...
        double test_int = 0;
         ...
         q_tilda = e(i);
         //test_int = test_int + KUKA_INVDYN::integral(0,0.7854,1000,i);
         test_int = test_int + KUKA_INVKIN::integral(0,q_tilda,10,i,pos);
         q_tilda = 0;
          }
         
        V2(0) = test_int;
        //V1 = 0.5*vel.transpose()*B*vel + 0.5*pos.transpose()*K*pos;
        V1 = 0.5*vel.transpose()*B*vel+V2;
	return V1;
...
VectorXd KUKA_INVKIN::forw_dy2(VectorXd pos,VectorXd vel,VectorXd v){
	...
	VectorXd L;
        ...
        double s = 0;
               ...
               for(int i = 0;i<7;i++){
                temp_tao = tao(i);
                s = KUKA_INVKIN::staturation(temp_tao,Ll[i],Mm[i]);
                tao_s(i) = s;
                s = 0;
               }   
                
                
                Eigen::VectorXd qdd = m2*(- coriol_.data - grav_.data + tao_s); 
...
double E_term = 150;
		     ...
		     for (int j=0;j<7;j++){
               ...
               //VectorXd V = KUKA_INVKIN::lyp(b_t2,B,e,b_t1);	 
              // cout<<"V: "<<V<<endl;
               //double Delta_V = k_e*(E_term - V(0));  
               double Delta_V = k_e*(E_term);
...
ofstream myfile_qd;
ofstream myfile_p;
myfile_qd.open("/home/utente/ros_ws/src/iiwa_kdl/src/ergs.m");
myfile_p.open("/home/utente/ros_ws/src/iiwa_kdl/src/pergs.m");
myfile_qd<<"q_ergs = [";
myfile_p<<"param_ergs = [";
       ...
       while( ros::ok() ) {	
               ...
               //Navigation Field Evaluation
                ...
                cout<<"NF: "<<ro.transpose()<<" / "<<"DSM_d: "<<Delta<<endl;
...
int main(int argc, char** argv) {
        ...
        int x;
        cout<<"ERGS CONTROL PRESS 1 TO START"<<endl;
        cin>>x;
	ros::init(argc, argv, "iiwa_kdl_ergs");
	KUKA_INVKIN ik;
	ik.run();
